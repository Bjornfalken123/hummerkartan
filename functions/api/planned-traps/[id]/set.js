...
export async function onRequestPost(context){
  try{
    ...
    const meta=positionMetaFromBody(body,lat,lon),event=positionEventStatement(db,'trap_set',trapId,meta,actor);if(event)statements.push(event);const tripEvent=tripEventStatement(db,body.trip_id,'trap_set',trapId,setAt,actor);if(tripEvent)statements.push(tripEvent);
