...
export async function onRequestPatch(context){
  try{
    ...
    const trap=await db.prepare('SELECT id FROM traps WHERE id=?').bind(trapId).first();
    ...
    const after={...current,trap_id:trapId,lobster_count:lobsterCount,released_count:releasedCount,notes,actor};const statements=[db.prepare('UPDATE checks SET trap_id=?,lobster_count=?,released_count=?,notes=?,actor=? WHERE id=?').bind(trapId,lobsterCount,releasedCount,notes,actor,id),recalcTrap(db,current.trap_id,actor,now),correctionEventStatement(db,'check',id,'update',current,after,actor)];
    if(trapId!==current.trap_id)statements.push(recalcTrap(db,trapId,actor,now));
