...
export async function onRequestGet(context) {
  try {
    ...
    const [traps,recentChecks,recentTrips,planned,positionEventsTable] = await Promise.all([
      db.prepare(`
        SELECT t.*,
    ...
    return json({
      ok:true,user:actorFromContext(context),traps:traps.results||[],checks:recentChecks.results||[],trips:recentTrips.results||[],planned_traps:planned.results||[],capabilities:{position_events:Boolean(positionEventsTable)},serverTime:new Date().toISOString()
    });
