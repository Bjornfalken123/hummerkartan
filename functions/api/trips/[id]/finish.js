...
export async function onRequestPost(context){
  try{
    const db=getDb(context),id=context.params.id,body=await readJson(context.request),ended=text(body.ended_at,isoNow())||isoNow(),distance=Math.max(0,finite(body.distance_nm,0)),actor=actorFromContext(context);
