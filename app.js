...
const TRAP_SOURCE_ID='hk-traps-source',TRAP_HALO_LAYER_ID='hk-traps-halo',TRAP_POINT_LAYER_ID='hk-traps-points',TRAP_LABEL_LAYER_ID='hk-traps-labels';
const PLANNED_SOURCE_ID='hk-planned-source',PLANNED_HALO_LAYER_ID='hk-planned-halo',PLANNED_POINT_LAYER_ID='hk-planned-points',PLANNED_LABEL_LAYER_ID='hk-planned-labels';
...
let gpsFixHistory=[],gpsFixWaiters=[],lastHandledFixTimestamp=null,schemaWarningShown=false;
...
function ensureMapPointLayers(){
  ...
  if(!map.getLayer(TRAP_HALO_LAYER_ID))map.addLayer({id:TRAP_HALO_LAYER_ID,type:'circle',source:TRAP_SOURCE_ID,filter:['==',['get','selected'],1],paint:{'circle-radius':18,'circle-color':'rgba(47,128,237,.18)','circle-stroke-width':0}});
  if(!map.getLayer(TRAP_POINT_LAYER_ID))map.addLayer({id:TRAP_POINT_LAYER_ID,type:'circle',source:TRAP_SOURCE_ID,filter:['==',['get','visible'],1],paint:{'circle-radius':12,'circle-color':['get','color'],'circle-stroke-color':'rgba(255,255,255,.96)','circle-stroke-width':2.5,'circle-opacity':1}});
  if(!map.getLayer(TRAP_LABEL_LAYER_ID))map.addLayer({id:TRAP_LABEL_LAYER_ID,type:'symbol',source:TRAP_SOURCE_ID,filter:['==',['get','visible'],1],layout:{'text-field':['get','label'],'text-size':10,'text-allow-overlap':true,'text-ignore-placement':true},paint:{'text-color':'#ffffff','text-halo-color':'rgba(5,24,39,.55)','text-halo-width':1}});
  if(!map.getSource(PLANNED_SOURCE_ID))map.addSource(PLANNED_SOURCE_ID,{type:'geojson',data:emptyFeatureCollection()});
  if(!map.getLayer(PLANNED_HALO_LAYER_ID))map.addLayer({id:PLANNED_HALO_LAYER_ID,type:'circle',source:PLANNED_SOURCE_ID,filter:['==',['get','selected'],1],paint:{'circle-radius':18,'circle-color':'rgba(94,199,223,.18)','circle-stroke-width':0}});
  if(!map.getLayer(PLANNED_POINT_LAYER_ID))map.addLayer({id:PLANNED_POINT_LAYER_ID,type:'circle',source:PLANNED_SOURCE_ID,paint:{'circle-radius':12,'circle-color':['case',['==',['get','selected'],1],'rgba(94,199,223,.92)','rgba(9,37,60,.82)'],'circle-stroke-color':'rgba(94,199,223,.98)','circle-stroke-width':['case',['==',['get','selected'],1],3,2.5]}});
  if(!map.getLayer(PLANNED_LABEL_LAYER_ID))map.addLayer({id:PLANNED_LABEL_LAYER_ID,type:'symbol',source:PLANNED_SOURCE_ID,layout:{'text-field':['get','label'],'text-size':10,'text-allow-overlap':true,'text-ignore-placement':true},paint:{'text-color':['case',['==',['get','selected'],1],'#082137','#dff6ff'],'text-halo-color':'rgba(5,24,39,.55)','text-halo-width':1}});
  if(!map.getSource(BOAT_SOURCE_ID))map.addSource(BOAT_SOURCE_ID,{type:'geojson',data:emptyFeatureCollection()});
  if(!map.getLayer(BOAT_HALO_LAYER_ID))map.addLayer({id:BOAT_HALO_LAYER_ID,type:'circle',source:BOAT_SOURCE_ID,paint:{'circle-radius':15,'circle-color':'rgba(94,199,223,.18)','circle-stroke-width':0}});
  if(!map.getLayer(BOAT_SYMBOL_LAYER_ID))map.addLayer({id:BOAT_SYMBOL_LAYER_ID,type:'symbol',source:BOAT_SOURCE_ID,layout:{'text-field':'▲','text-size':24,'text-rotate':['get','course'],'text-rotation-alignment':'map','text-allow-overlap':true,'text-ignore-placement':true},paint:{'text-color':'#5ec7df','text-halo-color':'rgba(4,18,30,.88)','text-halo-width':2}});
  if(!mapPointEventsBound){
    mapPointEventsBound=true;
    for(const layerId of [TRAP_POINT_LAYER_ID,TRAP_LABEL_LAYER_ID,PLANNED_POINT_LAYER_ID,PLANNED_LABEL_LAYER_ID]){
      map.on('mouseenter',layerId,()=>{map.getCanvas().style.cursor='pointer'});
      map.on('mouseleave',layerId,()=>{map.getCanvas().style.cursor=''})
    }
  }
}
function trapMapColor(trap){const cls=trapAgeClass(trap);if(cls==='checked-trip')return '#34b7d9';if(cls==='recent')return '#5ec7df';if(cls==='old')return '#5b87b6';if(cls==='retrieved')return '#667b8f';return '#2f80ed'}
function renderTrapMarkers(){
  if(!map)return;ensureMapPointLayers();const source=map.getSource(TRAP_SOURCE_ID);if(!source)return;
  const features=state.traps.map(trap=>{const lat=+trap.lat,lon=+trap.lon;if(!Number.isFinite(lat)||!Number.isFinite(lon))return null;const visible=(trap.status==='active'||(isDesktop()&&!desktopFilterActive))?1:0;return {type:'Feature',id:trap.id,properties:{id:trap.id,label:trapCheckedThisTrip(trap.id)?'✓':shortLabel(trap.name),name:trap.name||'Tina',color:trapMapColor(trap),selected:trap.id===selectedTrapId?1:0,visible},geometry:{type:'Point',coordinates:[lon,lat]}}}).filter(Boolean);
...
async function deletePlanned(id=selectedPlannedId){const planned=state.planned.find(p=>p.id===id);if(!planned)return;if(!confirm(`Ta bort ${planned.name}?`))return;await api('DELETE',`/api/planned-traps/${encodeURIComponent(id)}`,null,{queue:true});state.planned=state.planned.filter(p=>p.id!==id);if(selectedPlannedId===id)selectedPlannedId=null;saveState();renderAll();closeSheets();toast('Planen borttagen')}
...
async function requestSetTrap(plannedId=null){
  if(actionBusy)return;pendingSet={plannedId,name:nextTrapName()};$('setTrapName').textContent=pendingSet.name;$('setTrapPosition').textContent='Position tas när du trycker Sätt tina';const recent=currentFix&&Date.now()-currentFix.timestamp<=GPS_CHECK_MAX_AGE_MS;$('setTrapAccuracy').textContent=recent&&Number.isFinite(currentFix.accuracy)?`GPS ±${Math.round(currentFix.accuracy)} m`:'Väntar på aktuell GPS';$('confirmSetTrapBtn').textContent='Sätt tina';$('confirmSetTrapBtn').disabled=false;openSheet('setTrapSheet');
}

async function confirmSetTrap(){if(!pendingSet||actionBusy)return;const actionAt=Date.now();actionBusy=true;const btn=$('confirmSetTrapBtn'),oldText=btn.textContent;btn.disabled=true;btn.textContent='Läser GPS…';const planned=pendingSet.plannedId?state.planned.find(p=>p.id===pendingSet.plannedId):null;let success=false;try{const fix=await captureSetPosition(actionAt);$('setTrapPosition').textContent=`${fix.pos.lat.toFixed(5)}, ${fix.pos.lon.toFixed(5)}`;$('setTrapAccuracy').textContent=`GPS ±${Math.round(fix.accuracy)} m`;const body={id:crypto.randomUUID(),name:pendingSet.name,lat:+fix.pos.lat,lon:+fix.pos.lon,notes:planned?.notes||'',set_at:new Date(actionAt).toISOString(),...gpsMetaFields(fix,actionAt)};if(!activeTrip&&!isDesktop())await startTrip({quiet:true});if(planned)await api('POST',`/api/planned-traps/${encodeURIComponent(planned.id)}/set`,body,{queue:true});else await api('POST','/api/traps',body,{queue:true});state.traps.unshift({...body,status:'active',last_checked_at:null,created_at:isoNow(),updated_at:isoNow(),updated_by:state.user});if(planned){state.planned=state.planned.filter(p=>p.id!==planned.id);selectedPlannedId=null}selectedTrapId=body.id;saveState();renderAll();success=true;closeSheets();toast(`${body.name} satt · GPS ±${Math.round(fix.accuracy)} m`)}catch(error){$('setTrapPosition').textContent='Position kunde inte sparas';$('setTrapAccuracy').textContent=error.message||'Kontrollera platsbehörighet';if(String(error.message||'').includes('position_events'))toast('D1 behöver migration 0004_position_events.sql',5200);else toast(error.message||'Kunde inte sätta tina',3600)}finally{actionBusy=false;btn.disabled=false;btn.textContent=oldText;if(success)pendingSet=null}}
...
async function saveCheck(ev){ev.preventDefault();const trap=state.traps.find(x=>x.id===selectedTrapId);if(!trap||actionBusy)return;const actionAt=Date.now(),fix=reliableCheckFix(actionAt);actionBusy=true;try{if(!activeTrip&&!isDesktop())await startTrip({quiet:true});const body={id:crypto.randomUUID(),trap_id:trap.id,checked_at:new Date(actionAt).toISOString(),lobster_count:Math.max(0,Number($('lobsterCount').value||$('lobsterCount').textContent||0)),released_count:Math.max(0,Number($('releasedCount').value||0)),notes:$('checkNotes').value.trim(),lat:fix?.pos.lat??null,lon:fix?.pos.lon??null,...(fix?gpsMetaFields(fix,actionAt):{})};await api('POST','/api/checks',body,{queue:true});state.checks.unshift({...body,actor:state.user,trap_name:trap.name});trap.last_checked_at=body.checked_at;trap.updated_at=body.checked_at;saveState();renderAll();closeSheets();toast(`${trap.name} · ${body.lobster_count} hummer`);loadHeatmap({quiet:true});loadReports(reportYear,{quiet:true})}catch(error){if(String(error.message||'').includes('position_events'))toast('D1 behöver migration 0004_position_events.sql',5200);else toast(error.message||'Kunde inte spara vittjning',3600)}finally{actionBusy=false}}
...
function handlePosition(position){const fix=normalizeGpsFix(position);if(!fix)return;const duplicate=lastHandledFixTimestamp===fix.timestamp;rememberGpsFix(fix);let speed=fix.speed_kn;if(!Number.isFinite(speed)&&lastPosition&&fix.timestamp-lastPositionAt>1000){const d=distanceNm(lastPosition,fix.pos),hours=(fix.timestamp-lastPositionAt)/36e5;if(hours>0)speed=d/hours}currentPosition=fix.pos;currentAccuracy=fix.accuracy;updateBoat(fix.pos,fix.course);$('speedValue').textContent=Number.isFinite(speed)?clamp(speed,0,99).toFixed(1):'0.0';renderSelections();renderOverview();if(activeTrip&&!duplicate)appendTripFix(fix,speed);if(!duplicate){lastPosition=fix.pos;lastPositionAt=fix.timestamp;lastHandledFixTimestamp=fix.timestamp}}
...
async function startTrip({quiet=false}={}){if(activeTrip){if(!quiet)openSheet('tripSheet');return activeTrip}const trip={id:crypto.randomUUID(),name:`Hummertur ${new Date().toLocaleDateString('sv-SE')}`,started_at:isoNow()};await api('POST','/api/trips',trip,{queue:true});activeTrip=trip;tripPoints=[];trackBatch=[];trackSeq=0;tripDistanceNm=0;saveActiveTrip();renderTripUi();if(!quiet){toast('Turen startad');centerBoat()}return trip}
...
async function loadHeatmap({quiet=false}={}){try{const data=await api('GET','/api/heatmap');heatData={points:Array.isArray(data.points)?data.points:[],totals:data.totals||{checks:0,lobsters:0,average:0}};saveJson(HEAT_CACHE_KEY,heatData);updateHeatmapSource();if(!quiet)toast('Fångstkarta uppdaterad')}catch{heatData=loadJson(HEAT_CACHE_KEY,heatData);updateHeatmapSource();if(!quiet)toast('Visar cachad fångstkarta')}}
...
function initMap(){
  ...
  map.on('click',e=>{if(placementMode)return;const layers=[TRAP_LABEL_LAYER_ID,TRAP_POINT_LAYER_ID,PLANNED_LABEL_LAYER_ID,PLANNED_POINT_LAYER_ID].filter(id=>map.getLayer(id));const hit=(layers.length?map.queryRenderedFeatures(e.point,{layers}):[])[0];if(hit){const id=hit.properties?.id;if(hit.source===TRAP_SOURCE_ID&&id){selectTrap(id);if(isDesktop())openTrapDetail();return}if(hit.source===PLANNED_SOURCE_ID&&id){selectPlanned(id);if(isDesktop())openPlannedDetail();return}}clearSelection()});
}

bindUi();setTheme(theme);applyMobileMode();initMap();
syncTimer=setInterval(()=>syncState({quiet:true}),15000);setInterval(()=>{if(activeTrip)flushTrackBatch()},10000);if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=3.4.0').catch(()=>{});
