...
function bindUi(){
  ...
  $('trapEditForm').addEventListener('submit',saveTrapEdit);$('moveTrapBtn').addEventListener('click',()=>{const id=$('detailTrapId').value;closeSheets();beginPlacement(`move-trap:${id}`)});$('retrieveTrapBtn').addEventListener('click',retrieveTrap);$('plannedEditForm').addEventListener('submit',savePlannedEdit);$('movePlannedBtn').addEventListener('click',()=>{const id=$('detailPlannedId').value;closeSheets();beginPlacement(`move-planned:${id}`)});$('deletePlannedBtn').addEventListener('click',()=>deletePlanned($('detailPlannedId').value));
...
syncTimer=setInterval(()=>syncState({quiet:true}),15000);setInterval(()=>{if(activeTrip)flushTrackBatch()},10000);if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=3.6.0').catch(()=>{});
