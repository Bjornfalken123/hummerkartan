...
function renderTrapMarkers(){
  ...
  for(const trap of state.traps){
    ...
    if(!item){const el=document.createElement('button');el.type='button';el.className='trap-marker';el.setAttribute('aria-label',trap.name||'Tina');el.innerHTML='<span class="trap-marker-core"><span class="trap-marker-label"></span></span>';el.addEventListener('click',ev=>{ev.stopPropagation();selectTrap(trap.id);if(isDesktop())openTrapDetail()});const marker=new maptilersdk.Marker({element:el,anchor:'bottom',subpixelPositioning:true}).setLngLat([lon,lat]).addTo(map);item={el,marker};trapMarkers.set(trap.id,item)}else item.marker.setLngLat([lon,lat]);
    item.el.className=`trap-marker ${trapAgeClass(trap)}${trap.id===selectedTrapId?' selected':''}`;item.el.querySelector('.trap-marker-label').textContent=shortLabel(trap.name);
...
function updateBoat(pos,course){if(!map)return;if(!boatMarker){const el=document.createElement('div');el.className='boat-marker';el.innerHTML='<div class="boat-arrow"></div>';boatMarker=new maptilersdk.Marker({element:el,anchor:'center',subpixelPositioning:true}).setLngLat([pos.lon,pos.lat]).addTo(map)}else boatMarker.setLngLat([pos.lon,pos.lat]);const arrow=boatMarker.getElement().querySelector('.boat-arrow');if(arrow&&Number.isFinite(course))arrow.style.transform=`rotate(${course}deg)`}
