...
function initMap(){
  ...
  maptilersdk.config.apiKey=MAPTILER_API_KEY;map=new maptilersdk.Map({container:'map',style:maptilersdk.MapStyle.STREETS,center:WEST_COAST_CENTER,zoom:START_ZOOM,geolocateControl:false,attributionControl:true});map.addControl(new maptilersdk.NavigationControl({showCompass:false}),'bottom-right');
...
syncTimer=setInterval(()=>syncState({quiet:true}),15000);setInterval(()=>{if(activeTrip)flushTrackBatch()},10000);if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=3.0.0').catch(()=>{});
