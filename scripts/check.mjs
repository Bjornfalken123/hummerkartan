...
const required=[
  'index.html','app.js','gps.js','soak.js','styles.css','RELEASE-V3.6.md','RELEASE-V3.6.1.md','RELEASE-V3.6.2.md','depth.js','manifest.webmanifest','sw.js','boot.js','login.html','_headers',
...
if(!sw.includes('hummerkartan-shell-v17')) throw new Error('Service worker cache not bumped');
if(!html.includes('boot.js?v=3.6.2')||!html.includes('styles.css?v=3.6.2')||!app.includes("sw.js?v=3.6.2")) throw new Error('Asset version not bumped to 3.6.2');
if(!fs.readFileSync(path.join(root,'boot.js'),'utf8').includes("app.js?v=3.6.2")) throw new Error('Boot app version not bumped');
...
if(!checksApi.includes('client_trap_snapshot')||!checksApi.includes('check_locations')||!checksApi.includes('SELECT id FROM trips WHERE started_at<=?')) throw new Error('Checks API must snapshot trap location and infer historical trip');
...
console.log('Hummerkartan v3.6.2: soak-status, smart arbetsförslag, Fångstdata i båda lägen, enhetlig marin UI, Turer/efterarbete och PWA OK');
