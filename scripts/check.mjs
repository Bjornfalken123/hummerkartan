...
const required=[
  'index.html','app.js','gps.js','soak.js','styles.css','RELEASE-V3.6.md','RELEASE-V3.6.1.md','RELEASE-V3.6.2.md','RELEASE-V3.6.4.md','RELEASE-V3.6.5.md','depth.js','manifest.webmanifest','sw.js','boot.js','login.html','_headers',
...
if(!sw.includes('hummerkartan-shell-v20')) throw new Error('Service worker cache not bumped');
if(!html.includes('boot.js?v=3.6.5')||!html.includes('styles.css?v=3.6.5')||!app.includes("sw.js?v=3.6.5")) throw new Error('Asset version not bumped to 3.6.5');
if(!fs.readFileSync(path.join(root,'boot.js'),'utf8').includes("app.js?v=3.6.5")) throw new Error('Boot app version not bumped');
...
if(!heatmapApi.includes('COALESCE(cl.trap_lat,c.lat,t.lat)')||!heatmapApi.includes('COALESCE(c.lat,t.lat)')) throw new Error('Heatmap must support canonical checks.lat/lon with optional legacy snapshots');
if(!checksApi.includes('Fångstplatsen är alltid en snapshot')||!checksApi.includes('Number(trapLat),Number(trapLon)')||!checksApi.includes('Best-effort revisionsdata')) throw new Error('Checks API must persist trap snapshot as core catch location and keep metadata best-effort');
...
if(app.includes('NavigationControl')) throw new Error('MapTiler zoom controls must not be mounted');
if(!html.includes('menu-control')||!html.includes('Meny och inställningar')) throw new Error('High-contrast menu/settings control missing');
if(!fs.readFileSync(path.join(root,'styles.css'),'utf8').includes('v3.6.5 — interface rhythm')) throw new Error('v3.6.5 spacing system missing');
...
if(!stateApi.includes('check_locations:Boolean(checkLocationsTable)')) throw new Error('check_locations capability discovery missing');
...
console.log('Hummerkartan v3.6.5: Vittja core-data, heatmap-position, soak-status, smart arbetsförslag, Turer/efterarbete och PWA OK');
